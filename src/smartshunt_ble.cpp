...
constexpr size_t VICTRON_HEADER_LEN = 5;  // 0x10, record type, nonce LSB/MSB, key byte 0
...
bool parse_mac(const char *text, std::array<uint8_t, 6> &mac)
{
    ...
    if (std::sscanf(text, "%2x:%2x:%2x:%2x:%2x:%2x", &b[0], &b[1], &b[2], &b[3], &b[4], &b[5]) != 6) {
        return false;
    }
...
{
    if (!g_target_mac_valid) return true;
    return std::memcmp(mac, g_target_mac.data(), 6) == 0;
}
...
bool decrypt_payload(const uint8_t *encrypted, size_t encrypted_len, uint8_t nonce_lsb, uint8_t nonce_msb,
                     uint8_t *decrypted)
{
    ...
    const int rc = mbedtls_aes_crypt_ctr(&ctx, encrypted_len, &nc_offset, nonce_counter,
...
void publish_decoded(const uint8_t *record, int rssi, const uint8_t mac[6])
{
    ...
    if (ttg_raw != 0xFFFFU) next.time_to_go_min = static_cast<uint16_t>(ttg_raw);
    if (voltage_raw != 0x7FFFU) next.voltage_v = static_cast<float>(sign_extend(voltage_raw, 16)) * 0.01f;
    if (current_raw != 0x3FFFFFU) next.current_a = static_cast<float>(sign_extend(current_raw, 22)) * 0.001f;
    if (consumed_raw != 0xFFFFFU) next.consumed_ah = -static_cast<float>(consumed_raw) * 0.1f;
    if (soc_raw != 0x3FFU) next.soc_pct = static_cast<float>(soc_raw) * 0.1f;
...
{
    if (!g_settings.smartshunt_enabled || scan.search_evt != ESP_GAP_SEARCH_INQ_RES_EVT) return;
    if (!mac_matches(scan.bda)) return;
    ...
    if (!g_bindkey_valid) {
        ...
        pending.stale = false;
        pending.rssi = scan.rssi;
        std::strncpy(pending.source_mac, found_mac, sizeof(pending.source_mac) - 1);
        if (g_mutex != nullptr && xSemaphoreTake(g_mutex, 0) == pdTRUE) {
            g_data = pending;
            xSemaphoreGive(g_mutex);
        }
    ...
    if (payload[4] != g_bindkey[0]) {
        SmartShuntData wrong_key{};
        wrong_key.valid = false;
        wrong_key.key_valid = false;
        wrong_key.stale = false;
        wrong_key.rssi = scan.rssi;
        std::strncpy(wrong_key.source_mac, found_mac, sizeof(wrong_key.source_mac) - 1);
        if (g_mutex != nullptr && xSemaphoreTake(g_mutex, 0) == pdTRUE) {
            g_data = wrong_key;
            xSemaphoreGive(g_mutex);
        }
    ...
    if (!decrypt_payload(payload + VICTRON_HEADER_LEN, BATTERY_RECORD_LEN, payload[2], payload[3], decrypted)) {
        ESP_LOGW(TAG, "AES-CTR decrypt failed for %s", found_mac);
        return;
    }
...
void gap_callback(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT:
        if (param->scan_param_cmpl.status == ESP_BT_STATUS_SUCCESS) {
            ...
            ESP_LOGE(TAG, "BLE scan parameter setup failed: %d", param->scan_param_cmpl.status);
        }
        break;
...
void refresh_parsed_settings(const AppSettings &settings)
{
    ...
    if (!g_bindkey_valid && settings.smartshunt_enabled) {
        ESP_LOGW(TAG, "SmartShunt enabled but encryption key is missing or invalid");
    }
}
}
...
bool smartshunt_ble_start(const AppSettings &settings)
{
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        ESP_LOGW(TAG, "Unable to release Classic BT memory: %s", esp_err_to_name(err));
    }
...
SmartShuntData smartshunt_ble_get_data()
{
    ...
    if (copy.valid && !copy.stale) {
        // Each newly decoded packet is returned with age 0. Track the most recent observation locally.
        last_valid_us = esp_timer_get_time();
    }
    if (last_valid_us > 0) {
        copy.age_ms = static_cast<uint32_t>((esp_timer_get_time() - last_valid_us) / 1000);
